...
export const createUserController = async (req: Request, res: Response) => {
  ...
  if (!email) throw Boom.badRequest('Email is required')
  if (!password) throw Boom.badRequest('Password is required')
  if (!Object.values(UserRole).includes(role)) throw Boom.badRequest('Invalid role')
  if (role === 'store' && !storeName) throw Boom.badRequest('Store name is required')

  const user = await createUserService({ name, email, password, role, storeName })
